...
def parse_option():
    ...
    parser.add_argument('--num-workers', type=int, help="batch size for single GPU")
    ...
    parser.add_argument('--repeated-aug', action='store_true',
                        help="whether to use repeated augmentation")
...
def main(config):
    ...
    if dist.get_rank() == 0:
        if hasattr(model, 'flops'):
            ...
            n_parameters = sum(p.numel() for p in model.parameters() if p.requires_grad)
            logger.info(f"number of params: {n_parameters}")
            logger.info('flops need to be calculated')
    ...
    loss_scaler = NativeScalerWithGradNormCount(use_scale=config.TRAIN.LOSS_SCALER.USE_SCALE)
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        logger.info(f"EPOCH {epoch} saving takes {datetime.timedelta(seconds=int(save_time))}")
        ...
        if max_accuracy == acc1:
            if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
                save_checkpoint_ema(config, epoch, model_without_ddp, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger, None, None, name='best_acc_ori_ckpt.pth')
        if max_accuracy_ema == acc1_ema:
            if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
                save_checkpoint_ema(config, epoch, model_ema.ema, max_accuracy_ema, optimizer, lr_scheduler, loss_scaler, logger, None, None, name='best_acc_ema_ckpt.pth')
        # tmp_max_acc = max(max_accuracy, max_accuracy_ema)
        # if model_ema is not None:
        #     if tmp_max_acc == acc1_ema or tmp_max_acc == acc1:
        #         if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
        #             save_checkpoint_ema(config, epoch, model_without_ddp, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger, model_ema, max_accuracy_ema, name='best_ckpt.pth')
        # else:
        #     if tmp_max_acc == acc1:
        #         if dist.get_rank() == 0 and (epoch % config.SAVE_FREQ == 0 or epoch == (config.TRAIN.EPOCHS - 1)):
        #             save_checkpoint_ema(config, epoch, model_without_ddp, max_accuracy, optimizer, lr_scheduler, loss_scaler, logger, model_ema, max_accuracy_ema, name='best_ckpt.pth')
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, model_ema=None, model_time_warmup=50):
    ...
    if config.AMP_DTYPE == 'fp16':
        dtype = torch.float16
    elif config.AMP_DTYPE == 'bf16':
        dtype = torch.bfloat16
    else:
        raise NotImplementedError(f"Not support {config.AMP_DTYPE}")
    for idx, (samples, targets) in enumerate(data_loader):
        torch.cuda.reset_peak_memory_stats()
        if idx == 0:
            begin_epoch_time = time.time() - end
            begin_epoch_time_str = str(datetime.timedelta(seconds=int(begin_epoch_time)))
            logger.info('Begin epoch time {} at epoch {}'.format(begin_epoch_time_str, epoch))
        ...
        with torch.cuda.amp.autocast(enabled=config.AMP_ENABLE, dtype=dtype):
        ...
        if "scale" in loss_scaler.state_dict():
            ...
            loss_scale_value = torch.tensor(1.0, device=loss.device)
...
if __name__ == '__main__':
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        print(f"LOCAL_RANK, RANK and WORLD_SIZE in environ: {gpu}/{rank}/{world_size}")
    else:
        rank = -1
        world_size = -1
