...
def build_optimizer(config, model, logger, **kwargs):
    ...
    parameters, no_decay_names, has_decay_names = set_weight_decay(model, skip, skip_keywords)
    logger.info(f"No weight decay list: {no_decay_names}")
    logger.info(f"Has weight decay list: {has_decay_names}")
...
def set_weight_decay(model, skip_list=(), skip_keywords=()):
    ...
    return [{'params': has_decay},
            {'params': no_decay, 'weight_decay': 0.}], no_decay_names, has_decay_names
