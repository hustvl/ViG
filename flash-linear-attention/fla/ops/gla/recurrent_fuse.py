...
def bid_fused_recurrent_gla_fwd_kernel(
    ...
    NK: tl.constexpr,  # number of split in the K dimension
    ...
    inv_p_q = q + i_bh * s_qk_h + i_k * BK + \
        tl.arange(0, BK) + (T-1) * DK 
    inv_p_k = k + i_bh * s_qk_h + i_k * BK + \
        tl.arange(0, BK) + (T-1) * DK
    inv_p_v = v + i_bh * s_vo_h + i_v * BV + \
        tl.arange(0, BV) + (T-1) * DV
    inv_p_o = o + (i_bh + i_k * B * H + NK*B*H) * s_vo_h + i_v * BV + \
        tl.arange(0, BV) + (T-1) * DV
    ...
    if USE_GK:
        p_gk = gk + i_bh * s_qk_h + i_k * BK + \
            tl.arange(0, BK) + 0
        inv_p_gk = gk + B*H*s_qk_h+ i_bh * s_qk_h + i_k * BK + \
            tl.arange(0, BK) + (T-1) * DK 
    if USE_GV: # not use
...
def fused_recurrent_gla_fwd_kernel(
    ...
    for _ in range(0, T):
        ...
        _o = tl.sum(_o, axis=1) # [BV]
        tl.store(p_o, _o.to(p_o.dtype.element_ty), mask=mask_bv)
        p_q += -DK if REVERSE else DK # point to next token
...
class BidFusedRecurrentGLAFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, gk, gv, scale=None, initial_state=None, output_final_state=False, reverse=False):
        ...
        o = q.new_empty(2*NK, batch_size, n_heads, seq_len,
        ...
        # import ipdb;ipdb.set_trace()
        o = o.sum(0)/2 # why sum at NK dimension?
...
class FusedRecurrentGLAFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, gk, gv, scale=None, initial_state=None, output_final_state=False, reverse=False):
        ...
        o = o.sum(0) # why sum at NK dimension?
        ctx.save_for_backward(q, k, v, gk, gv, initial_state, o)
        ctx.scale = scale
        ctx.reverse = reverse
        # we do not need the gradient of the final state from the next chunk
