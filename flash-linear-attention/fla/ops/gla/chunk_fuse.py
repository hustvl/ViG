...
from .bid_scan import BidScanTriton
...
class FusedChunkGLAFunction(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, g, scale, initial_state, output_final_state):
        ...
        g = torch.empty_like(g, dtype=torch.float32) # batch_size n_heads seq_len d_head_k
        ...
        # prepare cumprod of gates at block level
        ...
        # this is outer forloop in Figure 7 of the paper
...
def batch_bid(x):
    ...
    """
    bid_x = torch.cat([x, x.flip(dims=[2])], dim=0)
    return bid_x
...
def batch_bid_1b1(x):
    ...
    """
    x_f,x_b = x.chunk(2)
    bid_x = torch.cat([x_f, x_b.flip(dims=[2])], dim=0)
    return bid_x

def fused_chunk_gla(
    q: torch.Tensor, #batch_size, n_heads, seq_len, d_head_qk
    k: torch.Tensor, #batch_size, n_heads, seq_len, d_head_qk
    v: torch.Tensor, #batch_size, n_heads, seq_len, d_head_v
    g: torch.Tensor, # F.logsigmoid(gk) / 16, 2*batch_size, n_heads, seq_len, d_head_qk
    scale: int = -1,
...
def seq_bid_fused_chunk_gla(
    ...
    # q, k, v = map(batch_bid, [q, k, v])
    g = batch_bid_1b1(g)
    q, k, v, g = map(lambda x: pad(x), [q, k, v, g_f])
    o_f, final_state = FusedChunkGLAFunction.apply(
        q, k, v, g, scale, initial_state, output_final_state)
    o_f = o_f[..., :seq_len, :]
    ...
    k_b = k.flip(dims=[2])
    v_b = v.flip(dims=[2])
    q_b,k_b,v_b,g_b = map(lambda x: pad(x), [q_b, k_b, v_b, g_b])
    o_b, _ = FusedChunkGLAFunction.apply(
        q_b, k_b, v_b, g_b, scale, initial_state, output_final_state)
    o_b = o_b[..., :seq_len, :]

    o = (o_f + o_b.flip(dims=[2])) / 2
    return o, final_state
def opt_batch_bid_fused_chunk_gla(
    q: torch.Tensor, #batch_size, n_heads, seq_len, d_head_qk
    k: torch.Tensor, #batch_size, n_heads, seq_len, d_head_qk
    v: torch.Tensor, #batch_size, n_heads, seq_len, d_head_v
    g: torch.Tensor, # F.logsigmoid(gk) / 16, 2*batch_size, n_heads, seq_len, d_head_qk
    ...
    if initial_state is not None:
        initial_state = initial_state.detach()
    seq_len = q.shape[-2]
    d_head_qk,d_head_qk,d_head_v = q.shape[-1],k.shape[-1],v.shape[-1]
    qkv = torch.cat([q,k,v],dim=-1)
    qkv = opt_batch_bid(qkv)
    q,k,v = qkv.split([d_head_qk,d_head_qk,d_head_v],dim=-1)
    # q, k, v = map(opt_batch_bid, [q, k, v])
    g = batch_bid_1b1(g)
    q, k, v, g = map(lambda x: pad(x), [q, k, v, g])
    o, final_state = FusedChunkGLAFunction.apply(
        q, k, v, g, scale, initial_state, output_final_state)
